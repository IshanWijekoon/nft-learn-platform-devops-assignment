...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['profile_picture'])) {
    ...
    $upload_dir = 'uploads/creator_pictures/';
    ...
    $new_filename = 'creator_' . $user_id . '_' . time() . '.' . $file_extension;
    $upload_path = $upload_dir . $new_filename;
    
    // Delete old profile picture if exists
    $old_pic_query = "SELECT profile_picture FROM creators WHERE id = '$user_id'";
    ...
    if (move_uploaded_file($file['tmp_name'], $upload_path)) {
        // Update database
        $update_query = "UPDATE creators SET profile_picture = '$upload_path' WHERE id = '$user_id'";
